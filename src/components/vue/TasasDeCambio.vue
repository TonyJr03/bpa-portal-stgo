...
const textoActualizacion = computed(() => {
  ...
  return ultimaActualizacion.value.toLocaleString(props.lang === 'en' ? 'en-US' : 'es-CU', {
    day: '2-digit',
    month: 'long',
    year: 'numeric',
    hour: '2-digit',
  ...
  <div class="w-full">
    ...
    >
      ...
      >
        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
        <path
          d="M12.983 8.978c3.955-.182 7.017-1.446 7.017-2.978c0-1.657-3.582-3-8-3c-1.661 0-3.204.19-4.483.515m-2.766 1.246A2.53 2.53 0 0 0 4 6c0 1.22 1.944 2.271 4.734 2.74"
        />
      ...
      <p class="text-sm text-muted">
        {{ tl('emptyMsg') }}
        <code class="font-mono text-xs bg-bpa-100 dark:bg-bpa-amber-800/40 px-1 rounded">{{
          tl('emptyCollection')
        }}</code>
        {{ tl('emptyPost') }}
      </p>
    ...
    <div v-else class="space-y-8">
      ...
      <div class="overflow-hidden rounded-xl border border-bpa-200 dark:border-bpa-amber-800 shadow-sm">
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-bpa-50 dark:bg-bpa-950/80 border-b border-bpa-200 dark:border-bpa-amber-800">
                ...
                <th class="text-left px-6 py-3 font-semibold text-default dark:text-default hidden sm:table-cell">
                  {{ tl('colName') }}
                </th>
            ...
            <tbody class="divide-y divide-bpa-100 dark:divide-bpa-800/50">
              ...
              >
                <td class="px-6 py-4">
                  <span
                    class="inline-flex items-center justify-center rounded-md bg-bpa-100 dark:bg-bpa-800/40 text-bpa-800 dark:text-bpa-200 font-bold text-xs px-2.5 py-1 min-w-[3rem]"
                  >
                    {{ tasa.moneda }}
                  </span>
                </td>
        ...
        <div
          class="px-6 py-3 bg-bpa-50 dark:bg-bpa-950/60 border-t border-bpa-200 dark:border-bpa-amber-800 flex items-center justify-between gap-4 flex-wrap"
        >
          <p v-if="textoActualizacion" class="text-xs text-muted">{{ tl('labelUpdated') }} {{ textoActualizacion }}</p>
          <button
            @click="cargarTasas"
            class="inline-flex items-center gap-2 rounded-lg text-sm font-semibold text-primary dark:text-primary hover:text-primary-600 transition-colors"
          >
            <!-- tabler:refresh -->
            <svg
              xmlns="http://www.w3.org/2000/svg"
      ...
      <div
        class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 bg-bpa-50 dark:bg-bpa-amber-950/30 px-5 py-4 text-sm text-default dark:text-default"
      >
        <!-- tabler:info-circle -->
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 overflow-hidden shadow-sm">
        <div
          class="bg-bpa-800 dark:bg-bpa-950 px-6 py-4 flex items-center gap-2 text-white border-b border-bpa-amber-800"
        >
            ...
            <div class="sm:col-span-2">
              ...
              >
                ...
                <div class="flex flex-col self-stretch border-l border-bpa-200 dark:border-bpa-amber-800">
                  <button
                    type="button"
                    @click="ajustar(conversor, 'monto', 0.1, 0)"
              ...
              <p class="text-xs text-white/70 mt-2">
                ...
                {{ tl('labelRateOf') }} {{ tasaSeleccionada?.moneda }} ({{
                  conversor.direccion === 'a_cup'
                    ...
                    : formatCup(tasaSeleccionada?.venta ?? 0)
                }}
                CUP)
              </p>
            </div>
