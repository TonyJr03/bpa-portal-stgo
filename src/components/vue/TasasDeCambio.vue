...
const etiquetaTiene = computed(() => {
  if (!conversor.monedaSeleccionada) return tUI('divisa');
  ...
  <div class="w-full">
    ...
    <div v-else-if="!tasasActivas.length" class="rounded-xl border border-bpa-100 dark:border-bpa-amber-800 bg-bpa-50 dark:bg-bpa-950/60 p-8 text-center">
      ...
      <p class="text-sm text-muted">
        {{ tUI('emptyMsg') }}
        <code class="font-mono text-xs bg-bpa-100 dark:bg-bpa-amber-800/40 px-1 rounded">{{ tUI('emptyCollection') }}</code>
        {{ tUI('emptyPost') }}
      </p>
    </div>
    ...
    <div v-else class="space-y-8">
      ...
      <div class="overflow-hidden rounded-xl border border-bpa-200 dark:border-bpa-amber-800 shadow-sm">
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-bpa-50 dark:bg-bpa-950/80 border-b border-bpa-200 dark:border-bpa-amber-800">
                <th class="text-left px-6 py-3 font-semibold text-default dark:text-default w-20">{{ tUI('colCurrency') }}</th>
                <th class="text-left px-6 py-3 font-semibold text-default dark:text-default hidden sm:table-cell">{{ tUI('colName') }}</th>
                <th class="text-right px-6 py-3 font-semibold text-default dark:text-default">{{ tUI('colBuy') }}</th>
                <th class="text-right px-6 py-3 font-semibold text-default dark:text-default">{{ tUI('colSell') }}</th>
              </tr>
            </thead>
        ...
        <div class="px-6 py-3 bg-bpa-50 dark:bg-bpa-950/60 border-t border-bpa-200 dark:border-bpa-amber-800 flex items-center justify-between gap-4 flex-wrap">
          <p v-if="textoActualizacion" class="text-xs text-muted">{{ tUI('labelUpdated') }} {{ textoActualizacion }}</p>
      ...
      <div class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 overflow-hidden shadow-sm">
        ...
        <div class="bg-bpa-800 dark:bg-bpa-950 px-6 py-4 flex items-center gap-2 text-white border-b border-bpa-amber-800">
          ...
          <span class="font-semibold text-sm">{{ tUI('labelConverter') }}</span>
        </div>
        ...
        <div class="p-6 bg-white dark:bg-bpa-950/60">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
            ...
            <div class="sm:col-span-2">
              <label class="block text-xs font-semibold text-muted uppercase tracking-wide mb-1.5">{{ tUI('labelAmount') }}</label>
            ...
            <div>
              <label class="block text-xs font-semibold text-muted uppercase tracking-wide mb-1.5">{{ tUI('labelCurrency') }}</label>
              <div class="relative">
            ...
            <div>
              <label class="block text-xs font-semibold text-muted uppercase tracking-wide mb-1.5">{{ tUI('labelDirection') }}</label>
          ...
          <div class="mt-5">
            <div v-if="resultadoConversor" class="rounded-xl bg-primary text-white px-6 py-5 text-center">
              <p class="text-xs font-semibold uppercase tracking-widest text-white/70 mb-1">{{ tUI('labelResult') }}</p>
              <p class="text-3xl font-bold tracking-tight">{{ resultadoConversor }}</p>
              <p class="text-xs text-white/70 mt-2">
                {{ tUI('labelCalculated') }}
                {{ conversor.direccion === 'a_cup' ? tUI('labelBuy') : tUI('labelSell') }}
                {{ tUI('labelRateOf') }} {{ tasaSeleccionada?.moneda }}
