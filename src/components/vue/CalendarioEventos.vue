 ...
 *
 * @paleta de tipos (coherente con CalendarioMiniWidget.vue):
...
<template>
  <div class="w-full">
    ...
    <div v-else-if="errorDB"
      ...
      <p class="text-sm text-red-600 dark:text-red-500 mb-4">El calendario de eventos no está disponible.</p>
      <button @click="cargarEventos"
    ...
    <div v-else-if="!hayEventos"
      class="rounded-xl border border-bpa-100 dark:border-bpa-800 bg-bpa-50 dark:bg-bpa-950/60 p-10 text-center">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
      ...
      <p class="font-semibold text-default dark:text-default mb-1">Sin eventos publicados</p>
      <p class="text-sm text-muted">
        Añada registros en la colección
        <code class="font-mono text-xs bg-bpa-100 dark:bg-bpa-800/40 px-1 rounded">eventos</code>
        de PocketBase con <code class="font-mono text-xs bg-bpa-100 dark:bg-bpa-800/40 px-1 rounded">publicado = true</code>.
      </p>
    </div>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between mb-4 gap-2">
        <button @click="mesAnterior"
          class="flex items-center justify-center w-9 h-9 rounded-lg text-muted hover:text-default dark:hover:text-default hover:bg-bpa-50 dark:hover:bg-bpa-800/40 transition-colors">
          <!-- tabler:chevron-left -->
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            class="w-5 h-5">
        ...
        <div class="flex items-center gap-3">
          ...
          <button @click="irAHoy"
            class="text-xs font-semibold px-2.5 py-1 rounded-full bg-bpa-100 dark:bg-bpa-800/40 text-primary dark:text-primary hover:bg-bpa-200 dark:hover:bg-bpa-700/50 transition-colors">
            Hoy
          </button>
        </div>
      ...
      <div class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 overflow-hidden shadow-sm">
        ...
        <div class="grid grid-cols-7 bg-primary border-b border-bpa-200 dark:border-bpa-amber-800">
          <div
            ...
            :key="dia"
            class="py-2.5 text-center text-xs font-semibold text-white uppercase tracking-wide"
          >
            {{ dia }}
          </div>
        </div>
        ...
        <div class="grid grid-cols-7 bg-white dark:bg-bpa-950/40">
          ...
          >
            <!-- Fondos diagonales cuando coexisten dos tipos -->
            <template v-if="tieneCoexistencia(dia) && eventoTop(dia) && eventoBottom(dia)">
              ...
              <div class="absolute inset-0 pointer-events-none border-t border-r"
                :class="CONFIG_TIPO[eventoTop(dia)!.tipo]?.borde" />
              <div class="absolute inset-0 pointer-events-none border-b border-l"
            ...
            <div class="relative w-full max-w-sm rounded-2xl shadow-2xl border border-bpa-200 dark:border-bpa-amber-800 bg-white dark:bg-bpa-950/80 overflow-hidden">
              ...
              <div class="divide-y divide-bpa-200 dark:divide-bpa-amber-800">
                ...
                >
                  ...
                  <p v-if="ev.descripcion" class="text-sm text-muted mt-1 leading-relaxed">
                    {{ ev.descripcion }}
                  </p>
                </div>
              </div>
