...
function eventoBottom(dia: DiaCalendario): Evento | null {
  ...
  return [...dia.eventos].sort((a, b) => (PRIORIDAD_BOTTOM[b.tipo] ?? 0) - (PRIORIDAD_BOTTOM[a.tipo] ?? 0))[0];
}
...
const fechaDetalle = computed(() => {
  ...
  return diaSeleccionado.value.fecha.toLocaleDateString(props.lang === 'en' ? 'en-US' : 'es-CU', {
    weekday: 'long',
    day: 'numeric',
    month: 'long',
    year: 'numeric',
      ...
      <div class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 overflow-hidden shadow-sm">
        ...
        >
          ...
          >
            ...
            />
            <div
              class="relative w-full max-w-sm rounded-2xl shadow-2xl border border-bpa-200 dark:border-bpa-amber-800 bg-white dark:bg-bpa-950/80 overflow-hidden"
            >
              <!-- Cabecera del panel -->
              <div
                class="flex items-center justify-between gap-3 px-5 py-4 border-b border-bpa-200 dark:border-bpa-amber-800"
              >
                <div>
