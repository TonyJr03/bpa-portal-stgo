...
const titulo  = (c: Comunicado) => props.lang === 'en' && c.titulo_en?.trim()  ? c.titulo_en  : c.titulo;
const resumen = (c: Comunicado) => props.lang === 'en' && c.resumen_en?.trim() ? c.resumen_en : c.resumen;
...
<template>
  ...
  <div v-else-if="errorDB" class="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 p-8 text-center">
    ...
    <button @click="cargar" class="mt-3 text-sm text-red-600 dark:text-red-400 underline">{{ tUI('retry') }}</button>
  </div>
  ...
  <div v-else-if="!comunicados.length" class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 bg-bpa-50 dark:bg-bpa-950/60 p-10 text-center">
    <p class="font-semibold text-default dark:text-default">{{ tUI('noPending') }}</p>
  </div>
  ...
  <div v-else class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
    <article v-for="c in comunicados" :key="c.id"
      ...
      <div class="overflow-hidden h-40 flex-shrink-0">
        <img v-if="urlImagen(c)" :src="urlImagen(c)!" :alt="titulo(c)"
      ...
      <div class="flex flex-col flex-1 p-5 gap-3">
        ...
        <h3 class="font-bold text-default dark:text-default text-base leading-snug line-clamp-2">{{ titulo(c) }}</h3>
        <p class="text-sm text-muted leading-relaxed line-clamp-3 flex-1">{{ resumen(c) }}</p>
