...
// ── Strings de interfaz pura + labels de selects (genéricos, NO van a .ts) ───
...
function badgeConfig(r: Registro): { label: string; badge: string } {
  if (r.nivel === 'alerta') return {
    label: t('news.level.alerta'),
    badge: 'bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300',
  };
  if (r.nivel === 'urgente') return {
...
const todosFiltrados = computed((): Registro[] => {
  ...
  if (q) {
    lista = lista.filter((r) =>
      normalizar(tf(r.titulo, r.titulo_en)).includes(q) ||
      normalizar(tf(r.resumen, r.resumen_en)).includes(q),
    );
  }
...
const textoPaginacion = computed(() =>
  `${tUI('page')} ${paginaActual.value} ${tUI('of')} ${totalPaginas.value}`,
);
...
watch(busqueda, () => {
  ...
  debounceTimer = setTimeout(() => { paginaActual.value = 1; }, 350);
});
watch(categoriaFiltro, () => { paginaActual.value = 1; });
...
<template>
  ...
  <div class="flex flex-col sm:flex-row gap-3 mb-8">
    ...
    <div class="relative flex-1">
      ...
      <input :value="busqueda" @input="busqueda = ($event.target as HTMLInputElement).value"
        type="text" :placeholder="tUI('searchPlaceholder')"
  ...
  <div v-if="cargando" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
    <div v-for="i in 6" :key="i" class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 bg-white dark:bg-bpa-950/60 overflow-hidden animate-pulse">
  ...
  <!-- ── ESTADO: Error ─────────────────────────────────────────────────── -->
  <div v-else-if="errorDB" class="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 p-12 text-center">
    <!-- tabler:alert-circle -->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-10 h-10 mx-auto mb-3 text-red-500"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg>
    <p class="font-semibold text-red-700 dark:text-red-400 mb-1">{{ tUI('errorTitle') }}</p>
    <p class="text-sm text-red-600 dark:text-red-500 mb-5">{{ tUI('errorMsg') }}</p>
    <button @click="cargarDatos" class="inline-flex items-center gap-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium px-5 py-2.5 transition-colors">
      <!-- tabler:refresh -->
  ...
  <div v-else-if="!hayResultados" class="rounded-xl border border-bpa-200 dark:border-bpa-amber-800 bg-bpa-50 dark:bg-bpa-950/60 p-12 text-center">
    <!-- tabler:news-off -->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-10 h-10 mx-auto mb-3 text-muted opacity-40"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M16 6h3a1 1 0 0 1 1 1v9m-.606 3.435A2 2 0 0 1 16 18v-2m0-4V5a1 1 0 0 0-1-1H8m-3.735.321A1 1 0 0 0 4 5v12a3 3 0 0 0 3 3h11M8 12h4m-4 4h4M3 3l18 18"/></svg>
    <p class="font-semibold text-default dark:text-default mb-1">{{ tUI('emptyTitle') }}</p>
    <p class="text-sm text-muted">
      <span v-if="busqueda || categoriaFiltro !== 'todas'">{{ tUI('emptyFilters') }}</span>
      <span v-else>{{ tUI('emptyNoContent') }}</span>
    </p>
  ...
  <template v-else>
    ...
    <p class="text-sm text-muted mb-6">
      ...
      {{ totalItems !== 1 ? tUI('resultPlural') : tUI('result') }}
      <span v-if="categoriaFiltro !== 'todas'">
        {{ tUI('resultIn') }} {{ t(`news.category.${categoriaFiltro}`).toLowerCase() }}s
      </span>
    </p>
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <article v-for="r in registros" :key="r.id"
        ...
        <div class="overflow-hidden h-40 flex-shrink-0">
          <img v-if="urlImagen(r)" :src="urlImagen(r)!" :alt="tf(r.titulo, r.titulo_en)"
        ...
        <div class="flex flex-col flex-1 p-5 gap-3">
          ...
          <!-- Enlace al detalle -->
          <a v-if="r.slug" :href="`/${props.lang}/actualidad/${r.slug}`"
            ...
            {{ tUI('readMore') }}
            <!-- tabler:arrow-right -->
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 12l14 0"/><path d="M13 18l6 -6"/><path d="M13 6l6 6"/></svg>
          </a>
        </div>
      </article>
    </div>
    ...
    <!-- Paginación -->
    <div v-if="totalPaginas > 1" class="flex items-center justify-between gap-4 mt-10 pt-6 border-t border-bpa-200 dark:border-bpa-amber-800">
      <button @click="irAPagina(paginaActual - 1)" :disabled="paginaActual === 1"
        ...
        <!-- tabler:arrow-left -->
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 12l14 0"/><path d="M5 12l6 6"/><path d="M5 12l6 -6"/></svg>
        {{ tUI('prev') }}
      </button>
      ...
      <button @click="irAPagina(paginaActual + 1)" :disabled="paginaActual === totalPaginas"
        ...
        {{ tUI('next') }}
        <!-- tabler:arrow-right -->
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 12l14 0"/><path d="M13 18l6 -6"/><path d="M13 6l6 6"/></svg>
      </button>
    </div>
