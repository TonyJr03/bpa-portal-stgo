...
function limpiarFiltros() { busqueda.value = ''; filtrosTipo.value = new Set(); }
function toggleExpansion(id: string) { expandidoId.value = expandidoId.value === id ? null : id; }
...
async function inicializarMapa() {
  ...
  const limiteProvincia = L.latLngBounds(
    L.latLng(19.70, -77.25),
    L.latLng(20.65, -75.20),
  );

  mapa = L.map(mapContainer.value, {
    center: CENTRO_PROVINCIA, zoom: ZOOM_INICIAL,
    zoomControl: true, minZoom: 10, maxZoom: 16,
    maxBounds: limiteProvincia, maxBoundsViscosity: 1.0,
  });
...
function alCambiarMunicipio() {
  busqueda.value = ''; filtrosTipo.value = new Set(); expandidoId.value = null;
  if (!mapa) return;
...
<template>
  <div class="w-full">
    ...
    <!-- ── ESTADO: Error ─────────────────────────────────────────────────── -->
    <div v-else-if="errorDB" class="flex flex-col items-center justify-center gap-4 py-20 rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400">
      <!-- tabler:alert-circle -->
    ...
    <template v-else>
      ...
      <div class="flex flex-wrap gap-3 mb-5">
        <div v-for="tipo in (['S','AN','CA','CAE','AT'] as const)" :key="tipo" class="flex items-center gap-1.5 text-xs font-medium text-muted">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 lg:h-[calc(100vh-13rem)]">
        ...
        <!-- Mapa Leaflet -->
        <div class="rounded-2xl border border-bpa-200 dark:border-bpa-amber-800 overflow-hidden min-h-[420px] lg:h-full mapa-wrapper">
        ...
        <div class="flex flex-col gap-4 lg:h-full min-h-[400px]">
          ...
          <div v-else class="flex-1 overflow-y-auto space-y-5 pr-1 min-h-0
                   ...
                   [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-bpa-50 dark:[&::-webkit-scrollbar-track]:bg-bpa-amber-800/30
                   [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-bpa-200 dark:[&::-webkit-scrollbar-thumb]:bg-bpa-amber-600
                   hover:[&::-webkit-scrollbar-thumb]:bg-bpa-400 dark:hover:[&::-webkit-scrollbar-thumb]:bg-bpa-amber-800">
            ...
            <div v-for="grupo in puntosAgrupados" :key="grupo.tipo">
              ...
              <div class="space-y-2">
                <div v-for="punto in grupo.items" :key="punto.id"
                  ...
                  :class="[CONFIG_TIPO[punto.tipo].borderColor, CONFIG_TIPO[punto.tipo].cardBg, expandidoId === punto.id ? 'shadow-md' : 'hover:shadow-sm']">
                  ...
                  <button @click="volarAPunto(punto)"
                    ...
                    :class="expandidoId === punto.id ? CONFIG_TIPO[punto.tipo].cardActive : CONFIG_TIPO[punto.tipo].cardHover">
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 min-w-0">
                        ...
                        <div v-if="punto.tipo === 'AT' && disponibilidadMap.get(punto.id)" class="inline-flex items-center gap-1 flex-shrink-0">
                          <span class="w-2 h-2 rounded-full" :class="CONFIG_ESTADO[disponibilidadMap.get(punto.id)!.estado].dot" />
                          <span class="text-[11px] font-medium" :class="CONFIG_ESTADO[disponibilidadMap.get(punto.id)!.estado].text">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                      ...
                      :class="[expandidoId === punto.id ? 'rotate-180' : '', punto.latitud && punto.longitud ? 'text-primary dark:text-primary' : 'text-muted']">
                  ...
                  <Transition
                    ...
                    <div v-if="expandidoId === punto.id"
                      ...
                      <div class="flex items-start gap-2 pt-2.5">
                        <!-- tabler:map-pin -->
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-muted"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9 11a3 3 0 1 0 6 0a3 3 0 0 0 -6 0" /><path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1 -2.827 0l-4.244 -4.243a8 8 0 1 1 11.314 0z" /></svg>
                      ...
                      <div v-if="punto.telefono" class="flex items-start gap-2">
                        <!-- tabler:phone -->
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-muted"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 4h4l2 5l-2.5 1.5a11 11 0 0 0 5 5l1.5 -2.5l5 2v4a2 2 0 0 1 -2 2a16 16 0 0 1 -15 -15a2 2 0 0 1 2 -2" /></svg>
                      ...
                      <div v-if="punto.horario" class="flex items-start gap-2">
                        <!-- tabler:clock -->
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-muted"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0" /><path d="M12 7v5l3 3" /></svg>
                      ...
                      <div v-if="punto.nota || punto.nota_en" class="flex items-start gap-2">
                        <!-- tabler:info-circle -->
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-muted"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0" /><path d="M12 9h.01" /><path d="M11 12h1v4h1" /></svg>
                      ...
                      <template v-if="punto.tipo === 'AT'">
                        <div v-if="disponibilidadMap.get(punto.id)"
                          ...
                          :class="{
                            'bg-green-50 dark:bg-green-950/30 border border-green-100 dark:border-green-900/50':     disponibilidadMap.get(punto.id)!.estado === 'operativo',
                            'bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/50':             disponibilidadMap.get(punto.id)!.estado === 'fuera_de_servicio',
                            'bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-100 dark:border-yellow-900/50': disponibilidadMap.get(punto.id)!.estado === 'sin_efectivo',
                          }">
                          <div class="flex items-center justify-between gap-2">
                            <div class="flex items-center gap-1.5">
                              <span class="w-2 h-2 rounded-full" :class="CONFIG_ESTADO[disponibilidadMap.get(punto.id)!.estado].dot" />
                              <!-- label de estado → diccionario -->
                              <span class="text-xs font-semibold" :class="CONFIG_ESTADO[disponibilidadMap.get(punto.id)!.estado].text">
                                {{ CONFIG_ESTADO[disponibilidadMap.get(punto.id)!.estado].label }}
                              </span>
                            </div>
